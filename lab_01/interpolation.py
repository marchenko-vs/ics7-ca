...
def divided_difference_hermite(x: list, y: list, dy: list) -> float:
    if len(x) == 2:
        if abs(x[1] - x[0]) > EPS:
            return (y[1] - y[0]) / (x[1] - x[0])
        return dy[0]
    return (divided_difference_hermite(x[:len(x) - 1], y[:len(x) - 1], dy[:len(x) - 1]) -
           divided_difference_hermite(x[1:], y[1:], dy[1:])) / (x[0] - x[len(x) - 1])
...
def interpolate_hermite(x: list, y: list, dy: list, nodes: int,
    ...
    for i in range(1, nodes + 1):
        ...
        for j in range(0, i):
            multiplier *= argument - x[j]
        multiplier *= divided_difference_hermite(x[:i + 1], y[:i + 1], dy[:i + 1])
        function += multiplier
    return function
